...
//! protocol version, the signature algorithm, the source IP, the timestamp, the
//! client key and all KEM offers (via `offers_hash`, which covers each offer's
//! suite id, ciphertext, nonce, wrapped key and early data) — so none of these
//! can be altered.
...
/// each [`ort_proto::SuiteOffer`] and are covered by `offers_hash`.
...
impl ConnMeta {
    /// Canonical byte encoding (24 bytes): `src_ip(16) || ts_be(8)`.
    pub fn canonical(&self) -> [u8; 24] {
        let mut out = [0u8; 24];
        out[..16].copy_from_slice(&self.src_ip);
        out[16..].copy_from_slice(&self.ts_millis.to_be_bytes());
        out
    }
    ...
    pub fn replay_tag(&self, offer_nonce: &[u8; 32]) -> [u8; 32] {
        ...
        buf[..24].copy_from_slice(&self.canonical());
...
/// `LABEL || version || sig_alg || src_ip || ts || client_pk || offers_hash`.
pub fn client_binding_msg(
    ...
    let mut msg = Vec::with_capacity(CLIENT_BINDING_LABEL.len() + 4 + 24 + client_pk.len() + 32);
...
pub fn sign_client_binding(
    ...
    let msg = client_binding_msg(PROTOCOL_VERSION, identity.suite_id(), cm, client_pk, offers_hash);
    identity.sign(&msg)
}
...
pub fn verify_client_binding(
    ...
    let msg = client_binding_msg(PROTOCOL_VERSION, sig_alg, cm, client_pk, offers_hash);
