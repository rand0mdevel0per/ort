...
with protocol-level replay protection. Two suites — **v1: ML-KEM-768 + ML-DSA-65**
(post-quantum) and **v2: DHKEM(X25519) + Ed25519** (audited classical) — are
...
offers both suites (`--suites v1,v2`) and signs with ML-DSA (`--sig-alg v1`); the
server adopts whichever offered suite it holds a key for, or replies `ServerReject`.

## With a certificate (strict verification)

```sh
# generate per-suite server key(s) + self-signed cert(s) binding the public key(s)
ortd gen-cert --out ./cert            # --suite all (default) | v1 | v2
...
#   --suites v2 --sig-alg v2   use the classical suite only
```
